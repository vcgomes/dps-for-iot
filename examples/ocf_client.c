...
#if 0
static void onDiscoverAckAutomatic(DPS_Publication* pub, uint8_t* payload, size_t len)
{
    struct map_entry update[] = {
        STR_ENTRY("value", "true"),
        { },
    };
    DPS_Publication *update_pub;
    DPS_RxBuffer rx;
    DPS_TxBuffer tx;
    DPS_Status ret;
    size_t i, array_size, size, href_len;
    char *di;
    uint64_t status;
    const char *topics[1] = { OIC_ENDPOINT_ALL };
    char topic_mine[strlen(OIC_ENDPOINT_DEVICE_PREFIX) + SD_ID128_STRING_MAX];
    const char match[] = "oic.r.switch.binary";
    char *href, *needle = NULL;
    int r;
    bool found = false;

    ret = DPS_RxBufferInit(&rx, payload, len);
    if (ret != DPS_OK) {
    ...
    ret = parseResult(&rx, &di, &status);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("parseResult failed: %s\n", DPS_ErrTxt(ret));
        return;
    }

    ret = CBOR_DecodeArray(&rx, &size);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("CBOR_DecodeArray failed: %s\n", DPS_ErrTxt(ret));
        return;
    }

    ret = CBOR_DecodeMap(&rx, &size);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("CBOR_DecodeArray failed: %s\n", DPS_ErrTxt(ret));
        return;
    }
    ...
    while (true) {
        size_t key_len, skip_len;
        char *key;
        uint8_t maj;


        ret = CBOR_DecodeString(&rx, &key, &key_len);
        if (ret != DPS_OK) {
            DPS_ERRPRINT("CBOR_DecodeString failed: %s\n", DPS_ErrTxt(ret));
            return;
        }

        if (strequal(key, key_len, "links")) {
            found = true;
            break;
        }

        ret = CBOR_Skip(&rx, &maj, &skip_len);
        if (ret != DPS_OK) {
            DPS_ERRPRINT("CBOR_DecodeString failed: %s\n", DPS_ErrTxt(ret));
            return;
        }
    ...
    /* 'links' array */
    ret = CBOR_DecodeArray(&rx, &array_size);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("CBOR_DecodeArray failed: %s\n", DPS_ErrTxt(ret));
        return;
    }

    while (array_size-- && !needle) {
        size_t key_len;
        char *key;
        ...
        while (size--) {
            ...
            if (strequal(key, key_len, "href")) {
                ret = CBOR_DecodeString(&rx, &href, &href_len);
                if (ret != DPS_OK) {
                    DPS_ERRPRINT("CBOR_DecodeString failed: %s\n", DPS_ErrTxt(ret));
                    return;
                }
            ...
            if (strequal(key, key_len, "rt")) {
                if (matchStringArray(&rx, match)) {
                    needle = strndupa(href, href_len);
                    break;
                }
                continue;
            }

            if (strequal(key, key_len, "if")) {
                /* Just consume the array */
                matchStringArray(&rx, NULL);
                continue;
            }

            ret = CBOR_Skip(&rx, NULL, NULL);
            if (ret != DPS_OK) {
                DPS_ERRPRINT("CBOR_Skip failed: %s\n", DPS_ErrTxt(ret));
                return;
            }
        }
    }

    r = snprintf(topic_mine, sizeof(topic_mine), OIC_ENDPOINT_DEVICE_PREFIX "%s", di);
    if (r < 0) {
    ...
    update_pub = DPS_CreatePublication(node);

    ret = DPS_InitPublication(update_pub, topics, 1, false, NULL,
        onUpdateAck);
    if (ret != DPS_OK) {
    ...
    len = buildRequest(&tx, "update", needle, NULL);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("buildRequest failed: %s\n", DPS_ErrTxt(ret));
        return;
    }

    ret = encodeMap(&tx, update);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("buildRequest failed: %s\n", DPS_ErrTxt(ret));
        return;
    }

    ret = DPS_Publish(update_pub, tx.base, tx.txPos - tx.base, 0);
    if (ret != DPS_OK) {
        DPS_ERRPRINT("DPS_Publish failed: %s\n", DPS_ErrTxt(ret));
        return;
    }
}
#endif
